        ...
        switch filterComparisonType {
            case let .Contains(strings, caseSensitive):
                matches = strings.filter {
                    string in
                    return caseSensitive ? value.containsString(string) : value.lowercaseString.containsString(string.lowercaseString)
                }.count > 0
            ...
            case let .StartsWith(strings, caseSensitive):
                matches = strings.filter {
                    string in
                    return caseSensitive ? value.hasPrefix(string) : value.lowercaseString.hasPrefix(string.lowercaseString)
                }.count > 0

            case let .EndsWith(strings, caseSensitive):
                matches = strings.filter {
                    string in
                    return caseSensitive ? value.hasSuffix(string) : value.lowercaseString.hasSuffix(string.lowercaseString)
                }.count > 0

            case let .Equals(strings, caseSensitive):
                matches = strings.filter {
                    string in
                    return caseSensitive ? value == string : value.lowercaseString == string.lowercaseString
                }.count > 0
        }
